...
interface Review {
  ...
  rate?: number; // For API compatibility
  comment?: string;
  response?: string; // For API compatibility
  date?: string;
  createdAt?: number; // For API compatibility
  replyDate?: string;
...
const DanhGia: React.FC = () => {
  ...
  // Calculate rating statistics whenever reviews change
  useEffect(() => {
  ...
  const handleSubmitReview = async (values: any) => {
    ...
    try {
      ...
      const overall = (values.serviceRating + values.attitudeRating + values.cleanlinessRating + values.priceRating) / 4;
      ...
      // Prepare review for API
      const newReview = {
        id: Date.now().toString(), // Ensure unique ID for local state management
        rate: Math.round(overall * 20), // Convert to 0-100 scale for API
      ...
      // Update service to mark as reviewed
      await axios.put(`${SERVICE_API_URL}/${selectedService.id}`, {
        ...selectedService,
        user_review: true
      });
      
      // Update local state before fetching from server to ensure immediate UI update
      setReviews(prevReviews => [...prevReviews, response.data]);
      setUnreviewedServices(prevServices => 
  ...
  const showReviewModal = (service: Service) => {
    ...
    form.resetFields(); // Reset form when opening
  };
  ...
  // Get normalized rating value (1-5 scale)
  const getNormalizedRatingValue = (review: Review): number => {
    ...
    if (review.rate !== undefined) {
      return review.rate / 20; // Convert from 0-100 to 0-5 scale
    }
    return 0;
  };
              ...
              <List.Item
                actions={[
                  <Button 
                    type="primary" 
                    icon={<StarOutlined />}
                    onClick={() => showReviewModal(service)}
                  >
                    Đánh giá
                  </Button>
                ]}
                ...
                <List.Item.Meta
                  ...
                  description={
                    <>
                      ...
                      <br />
                      <Text type="secondary">
                        Thời gian thực hiện: {service.process_time}
                      </Text>
                    </>
                  }
                />
            ...
            <List
              ...
              renderItem={review => (
                <List.Item>
                      ...
                      <>
                        <Space direction="vertical" style={{ width: '100%' }}>
                          <Space>
                            ...
                            <Rate 
                              disabled 
                              value={getNormalizedRatingValue(review)}
                              allowHalf
                            />
                  ...
                  {/* Reply button for employee - only show if no response yet */}
                  {!review.response && (
                    <Button
                      type="link"
