	...
	const columns = [
			...
			render: (budget: number, record: BudgetCategory) => (
				<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
					...
					<Tooltip title='Chỉnh sửa ngân sách'>
						<Button type='text' size='small' icon={<EditOutlined />} onClick={() => showEditModal(record, 'budget')} />
					</Tooltip>
				</div>
			),
		},
			...
			render: (spent: number, record: BudgetCategory) => (
				<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
					...
					<Tooltip title='Cập nhật chi tiêu'>
						<Button type='text' size='small' icon={<EditOutlined />} onClick={() => showEditModal(record, 'spent')} />
					</Tooltip>
				</div>
			),
		},
		{
			...
			render: (_: any, record: BudgetCategory) => {
				...
				return <Text type={isOverBudget ? 'danger' : 'success'}>{formatCurrency(remaining)}</Text>;
			},
		},
		{
			title: 'Trạng thái',
