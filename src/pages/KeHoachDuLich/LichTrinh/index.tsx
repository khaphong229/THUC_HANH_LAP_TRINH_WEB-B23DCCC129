...
import type { Itinerary, ItineraryItem } from '@/services/KeHoachDuLich/LichTrinh/typings';
...
const LichTrinhPage: React.FC = () => {
	const {
		...
		onDragEnd,
	} = useModel('kehoachdulich.lichtrinh');
	...
	return (
		<div style={{ padding: '24px' }}>
			...
			<div style={{ display: 'flex', gap: '24px' }}>
				<div style={{ width: '300px' }}>
					...
					{currentItinerary && sortedItems.length > 0 && (
						<Card title='Tóm tắt lịch trình theo ngày' style={{ marginTop: 16 }} size='small'>
							<List
								size='small'
								dataSource={getUniqueDays()}
								renderItem={(day) => {
									const dayItems = sortedItems.filter((item) => item.day === day);
									const dayDestinations = dayItems
										.map((item) => destinations.find((d) => d.id === item.destinationId)?.name)
										.filter(Boolean);
										...
										>
											<div style={{ width: '100%' }}>
												...
												<div style={{ fontSize: '12px' }}>
													{dayDestinations.length > 0 ? dayDestinations.join(', ') : 'Chưa có điểm đến'}
												</div>
											</div>
										</List.Item>
									);
							...
							<Card
								...
								extra={
									<Space>
										...
										<Button icon={<EditOutlined />} onClick={handleEditItinerary}>
											Sửa thông tin
										</Button>
									</Space>
								}
							>
								<div style={{ display: 'flex', marginBottom: 16 }}>
									<div style={{ marginRight: 16 }}>
										...
										<Select value={currentDay} onChange={setCurrentDay} style={{ width: 200 }}>
									...
									<div style={{ display: 'flex', flex: 1 }}>
										...
										<Button type='primary' icon={<PlusOutlined />} onClick={handleAddDestination}>
											Thêm
										</Button>
									</div>
								</div>
			...
			<Modal title='Tạo lịch trình mới' visible={isModalOpen} onCancel={() => setIsModalOpen(false)} footer={null}>
				<Form form={form} layout='vertical' onFinish={handleCreateItinerary}>
					...
					<Form.Item name='totalBudget' label='Ngân sách dự kiến (VND)'>
			...
			<Modal
				...
				footer={null}
			>
				<Form form={editForm} layout='vertical' onFinish={handleUpdateItinerary}>
					<Form.Item
						name='name'
						label='Tên lịch trình'
						rules={[{ required: true, message: 'Vui lòng nhập tên lịch trình' }]}
